...
- 未完成：机制③"有界复活"只有设计；预测器的独立收益未证明（oracle 臂实现有缺陷、结论已撤回）；Concur 控制律已实现；论文参数 (0.2, 0.5) 的公平重测正在排队（此前观察到的“原参数停滞”已归因为我们的陈旧日志缺陷，见 STATE.md）；关键配置的重复实验暂不做（效率优先）。
...
| `scripts/controller_aimd2.py` | Concur 控制律复现（u_low/u_high/h_thresh + `--h-mode`）；原论文参数在 2026-09-20 已修复陈旧日志缺陷并排队公平重测，见 `docs/STATE.md` |
