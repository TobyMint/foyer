...
#
# 2026-09-19: the paper-params arm (u_low=0.2 + u_high=0.5) deadlocked a SECOND time,
# deterministically: controller logs W pinned at 1, U=0.0, active_n=0, action=hold,
# nothing ever admitted, engine idle for >1h (run killed at attempt 1). The run dir is
# preserved in results/night/_deadlock_evidence/aimd2_paper_20260919_attempt1/.
# The arm is removed from this lane so the retry can finish the two arms that still
# carry information; re-running the paper-params arm is a deliberate experiment of its
# own (it needs a short timeout, not 16h).
...
bash /data/xbw/turnstile/scripts/lane_done.sh aimd2_paper aimd2_uh06 aimd2_paper_rawH
