...
We replay 200 coding-agent sessions drawn from real traces. Each session is a
...
gap is 16.85\,s mean against a demanded 15.82\,s, a ratio of 1.001. This
matches the source corpus's own reported mean tool time of 16.8\,s.

\subsection{Engine and the binding constraint}

We use SGLang v0.5.10 with Qwen2.5-Coder-7B-Instruct extended to 96K context,
on one RTX 3090 (24\,GB). At \texttt{mem-fraction-static} 0.85 the KV pool is
101{,}432 tokens. Hierarchical caching (HiCache) is enabled throughout, with a
...
line between them. So we take the upper-left envelope of the static points and
report each controller's signed distance above or below it. Every verdict in
