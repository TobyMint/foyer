...
We explain why, and the explanation is the contribution. We decompose the
end-to-end wall clock into \emph{busy} and \emph{idle} time, and show that the
busy component is governed by two quantities the admission decision does not
control: the per-step cost and the number of requests actually in flight.
Admission policy moves the second---but the first rises with it, so the two
cancel. The idle component (7.4--36.3\% of the wall clock) is structural: an
