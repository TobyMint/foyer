...
The difficulty is that $\tau$ is not constant in $\overline{R}$: it rises from
18.4 to 32.2\,ms as $\overline{R}$ goes from 0.64 to 1.62, and that rise
absorbs more than half of what the extra concurrency buys.
...
the engine is genuinely empty---and we measure that the median zero-occupancy
stretch is 11\,s and 30\% exceed 20\,s, an order of magnitude above the 2\,s
control cycle. Crucially, the tool wait itself is much longer than
...
\begin{itemize}
  ...
  \item We give a two-part decomposition of the wall clock (\S\ref{sec:model})
        that locates the cancellation: admission policy moves the
        running-request count, and the per-step cost moves with it. We also
        report the mechanism we proposed for that rise and had to withdraw,
        with the two engine counters that refute it (\S\ref{sec:model}); the
        negative result does not rest on it, because the cancellation is
  ...
  \item We characterize when adaptivity \emph{could} have paid, and show that
        ...
        control does not reach it either.
  \item We make the measurements reproducible: the verdict of every run is
        ...
        the five claims we had to retract along the way, with the checks that
        caught them.
\end{itemize}
